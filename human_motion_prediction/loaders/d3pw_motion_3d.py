...
class D3PW_Motion3D(Dataset):  # This class must be cleaned, e.g. dct_n was removed.
    def __init__(self, path_to_data, actions, **kwargs):
        ...
        actions = data_utils.define_actions_pw3d(actions)  # actions = ['walking']
        is_test = False if split == "train" else True

        self.path_to_data = Path(self.path_to_data)
        all_seqs, dim_ignore, dim_use, class_seq, data_mean, data_std = data_utils.load_data_3dpw(self.path_to_data,
                                                                                                  actions,
                                                                                                  input_n, output_n,
                                                                                                  data_std=data_std,
                                                                                                  data_mean=data_mean,
